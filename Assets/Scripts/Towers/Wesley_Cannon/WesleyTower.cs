...
// Updaten naar WesleyTower, copy-pasted vanuit S1Tower
{
    ...
    [SerializeField] private ProjectileBase ActiveProjectile;
    ...
    private float PersonalFireRate = 0.5f;
    ...
    void Update()
    {
        ...
        {
            Cannon.transform.LookAt(EnemyToTarget.transform.position + new Vector3(0, Cannon.transform.position.y, 0));
            if (canFire)
            {
                _Animator.ResetTrigger("Shoot");
                _Animator.SetTrigger("Shoot");
                DoDamage(EnemyToTarget);
                canFire = false;
    ...
    {
        ActiveProjectile = ProjectileList[0];
    }
